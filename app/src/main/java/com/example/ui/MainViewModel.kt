...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val supabaseUrl = MutableStateFlow(AppRepository.SUPABASE_URL)
    val supabaseAnonKey = MutableStateFlow(AppRepository.SUPABASE_KEY)
    ...
    init {
        ...
        viewModelScope.launch {
            repository.adSettings.collect { settings ->
                if (settings != null) {
                    ...
                    supabaseUrl.value = AppRepository.SUPABASE_URL
                    supabaseAnonKey.value = AppRepository.SUPABASE_KEY
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            try {
                ...
                android.util.Log.e("ViewModel", "populateInitialData error: ${e.message}")
            }
            try {
                repository.syncAllTablesFromSupabase(AppRepository.SUPABASE_URL, AppRepository.SUPABASE_KEY)
            } catch (e: Exception) {
                android.util.Log.e("ViewModel", "initialSync error: ${e.message}")
            }
        }

        // Real-time background sync loop (every 30 seconds)
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            while (true) {
                ...
                if (_isLoggedIn.value) {
                    try {
                        ...
                        android.util.Log.e("ViewModel", "bgSync error: \${e.message}")
                    }
                }
            }
        }
